...
for name, load_f in all_data.items():
    ...
    for i in range(n_split):
        ...
        print("Xtrain_u.shape: ", Xtrain_u.shape
              , "Xval_u.shape: ", Xval_u.shape
              , "Xtest_u.shape: ", Xtest_u.shape
        ...
        model_mse = fit_model_NLL(Xtrain,Ytrain,nn=MLP_mean_var ,\
        ...
        nlls_rescaled.append(NLL(Ypred_test,Ytest,Ypred_test_var_iso_rescaled,eps=0.,full=True).detach())
        coeffs.append(get_coeff(Ypred_test,Ytest,Ypred_test_var).detach())
        coeffs_w_alpha.append(get_coeff(Ypred_test,Ytest,Ypred_test_var*alpha).detach()) #full = True, 
