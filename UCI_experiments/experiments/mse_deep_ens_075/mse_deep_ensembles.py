...
#all_data = {"years":load.load_yearsMSD,}#"concrete":load.load_concrete,"housing":load.load_housing,"wine":load.load_wine,
            # "yacht": load.load_yacht,}
# "years":load.load_yearsMSD,}#"naval":load.load_naval,"yacht": load.load_yacht}


with open("results_mse.txt","w") as fg:
...
for name, load_f in all_data.items():
    ...
    for i in range(n_split):
        ...
        model_mse, mask_08 = fit_deepensemble_mse_sequential(Xtrain,Ytrain,nn=MLP_mean ,\
                    model_kwargs=NN_KWARGS,solver=torch.optim.Adam,\
                    solver_kwargs=SOLVER_KWARGS,training_kwargs=TRAINING_KWARGS,n_ensemble=5,p_subsample=.75)
    ...
    with open("results_mse.txt","a") as fg:
